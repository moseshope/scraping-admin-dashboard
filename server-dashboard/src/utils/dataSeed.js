    ...
    const promises = chunkGroup.map(async (chunk) => {
      ...
      try {
        ...
        if (completedChunks % 100 === 0 || completedChunks === totalChunks) {
          ...
          log(
            `Progress: ${progress}% (${completedChunks}/${totalChunks} chunks) | Rate: ${rate} chunks/min | Est. total time: ${estimatedTotalMinutes} min`
          );
        }
        return chunk.length;
      } catch (error) {
...
async function seedData() {
  ...
  try {
    ...
    log(
      `Successfully seeded ${records.length} records to DynamoDB in ${totalTime} minutes`
    );
  } catch (error) {
