...
const { createProjectSchema, updateProjectStatusSchema, validateResults } = require('../utils/validation.schemas');

// Create a new project
router.post('/', [verifyToken, createProjectSchema, validateResults], async (req, res) => {
  try {
...
router.put('/:id', [
  verifyToken,
  createProjectSchema.map(validation => ({
    ...validation,
    optional: true
  })),
  validateResults
...
router.post('/:id/status', [
  ...
  try {
    const project = await projectModel.updateProjectStatus(req.params.id, status, success);
    if (!project) {
      return res.status(404).json({ error: 'Project not found' });
    }
